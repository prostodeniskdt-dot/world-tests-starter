...
export async function POST(
  ...
  try {
    const result = await db.query(
      `UPDATE equipment_reviews SET status = 'approved', reviewed_at = now(), reviewed_by = $1 WHERE id = $2 AND status = 'pending'`,
    ...
    console.error("Approve equipment review error:", err);
