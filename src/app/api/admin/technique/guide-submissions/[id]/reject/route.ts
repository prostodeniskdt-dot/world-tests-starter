...
export async function POST(
  ...
  try {
    const result = await db.query(
      `UPDATE technique_guide_submissions SET status = 'rejected', reviewed_at = now(), reviewed_by = $1 WHERE id = $2 AND status = 'pending'`,
    ...
    console.error("Reject guide submission error:", err);
