...
export async function POST(
  ...
  try {
    const result = await db.query(
      `UPDATE glassware_submissions SET status = 'rejected', reviewed_at = now(), reviewed_by = $1 WHERE id = $2 AND status = 'pending'`,
    ...
    console.error("Reject glassware submission error:", err);
