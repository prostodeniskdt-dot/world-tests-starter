...
import {
  ...
  normalizeScore1to5,
} from "@/lib/glasswarePayloadHelpers";
...
export async function POST(req: NextRequest) {
  ...
  if (categoryIdRaw != null && categoryIdRaw !== "") {
    ...
    if (!Number.isNaN(n) && n > 0) {
      const c = await db.query(`SELECT id FROM glassware_categories WHERE id = $1`, [n]);
      if (c.rows.length === 0) {
  ...
  const subcategory_text = trimText(body.subcategory_text ?? body.subcategoryText, 200);
  const producer = trimText(body.producer, 200);
  const material = trimText(body.material, 200);
  const volume_ml_raw = body.volume_ml ?? body.volumeMl;
  const volume_ml =
    volume_ml_raw != null && volume_ml_raw !== ""
      ? parseInt(String(volume_ml_raw), 10)
      : null;
  const dimensions = trimText(body.dimensions, 300);
  ...
  const ideal_for_drinks = trimText(body.ideal_for_drinks ?? body.idealForDrinks, 4000);
  ...
  const practicality_score = normalizeScore1to5(body.practicality_score ?? body.practicalityScore);
  const aesthetics_score = normalizeScore1to5(body.aesthetics_score ?? body.aestheticsScore);
  const durability_score = normalizeScore1to5(body.durability_score ?? body.durabilityScore);
  const tags = normalizeTags(body.tags);

  if (!description && !experience_pros) {
    return NextResponse.json(
      {
        ok: false,
        error: "Добавьте описание и/или плюсы использования",
      },
      { status: 400 }
    );
  }
  ...
  try {
    await db.query(
      `INSERT INTO glassware_submissions (
        user_id, category_id, name, slug, image_url, gallery_urls, description,
        subcategory_text, producer, material, volume_ml, dimensions,
        ...
        tags, related_knowledge_article_id, photo_rights_confirmed, status
      ) VALUES (
        $1, $2, $3, $4, $5, $6::jsonb, $7,
        $8, $9, $10, $11, $12,
        $13, $14, $15::jsonb, $16,
      ...
      [
        ...
        producer,
        material,
        volume_ml != null && !Number.isNaN(volume_ml) ? volume_ml : null,
        dimensions,
        price_segment,
        price_range,
    ...
    console.error("Glassware submit error:", err);
