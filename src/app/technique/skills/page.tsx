...
export default function TechniqueSkillsListPage() {
  ...
  const load = useCallback(() => {
    ...
    fetch(`/api/catalog/skills${qs ? `?${qs}` : ""}`)
      .then((r) => r.json())
      .then((data) => {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-6 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-600 leading-relaxed">
            Пошаговые гайды и практические советы
          </p>
        </div>
        <Link
      ...
      <div className="flex flex-col sm:flex-row flex-wrap gap-3 mb-6">
        {categories.length > 0 && (
          <select
            ...
            className="rounded-lg border border-zinc-300 px-3 py-2 text-sm bg-white min-w-[240px]"
          >
      ...
      {loading ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="rounded-xl border border-zinc-200 bg-white h-36 animate-pulse" />
          ))}
        </div>
      ) : items.length === 0 ? (
        <CatalogEmpty
          icon={GraduationCap}
          title="Пока пусто"
          description="Материалы появятся после модерации заявок."
        />
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((item) => (
            ...
            >
              ...
              {catName(item.category_id) ? (
                <p className="text-[11px] text-primary-700 mb-1">{catName(item.category_id)}</p>
              ) : null}
              {item.short_description ? (
                <p className="text-sm text-zinc-600 line-clamp-3">{item.short_description}</p>
              ) : null}
            </Link>
          ))}
        </div>
