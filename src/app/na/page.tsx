...
export default function NAPage() {
  ...
  const applyTag = () => setTagFilter(tagDraft.trim().toLowerCase());
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-600 leading-relaxed">
            Сиропы, пюре, тоники, соки и другие ингредиенты для барной витрины
          </p>
        </div>
        <Link
          href="/na/submit"
      ...
      <div className="flex flex-col sm:flex-row flex-wrap gap-3 mb-6">
        ...
        <div className="flex gap-2 min-w-[180px] max-w-xs">
          <input
            type="text"
            value={tagDraft}
            onChange={(e) => setTagDraft(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && applyTag()}
            placeholder="Тег (точное совпадение)"
      ...
      {loading ? (
        ...
        <CatalogEmpty
          ...
          description="Измените фильтры или предложите свою карточку ингредиента."
        />
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {items.map((item) => {
              ...
              >
                <div className="aspect-square bg-zinc-100 flex items-center justify-center shrink-0">
                  {item.image_url ? (
                    ...
                    <Coffee className="h-12 w-12 text-zinc-300" />
                  )}
                </div>
                <div className="p-3 flex flex-col flex-1">
                  ...
                  {cat ? <p className="text-[11px] text-zinc-500 mt-1">{cat}</p> : null}
                  {item.subcategory_text ? (
                    <p className="text-xs text-zinc-500 mt-0.5 line-clamp-1">{item.subcategory_text}</p>
                  ) : null}
                  {item.tags && item.tags.length > 0 ? (
                    <p className="text-[10px] text-zinc-400 mt-1 line-clamp-1">{item.tags.join(" · ")}</p>
                  ) : null}
                  {item.description ? (
