...
export default function AdminCocktailsPage() {
  ...
  const [classicFilter, setClassicFilter] = useState<"" | "true" | "false">("");
  ...
  const load = useCallback(() => {
    ...
    if (classicFilter) params.set("classic", classicFilter);
    if (categoryFilter !== "") params.set("category", String(categoryFilter));
    const qs = params.toString();
    fetch(`/api/admin/cocktails${qs ? `?${qs}` : ""}`)
      .then((r) => r.json())
      .then((d) => { if (d.ok) setItems(d.items || []); })
      .catch(() => {})
      .finally(() => setLoading(false));
  }, [q, classicFilter, categoryFilter]);
    ...
    <div className="min-h-screen bg-zinc-50">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          ...
          <Link
            href="/admin/cocktails/categories"
            className="text-sm text-primary-600 hover:underline font-medium"
          >
            Категории
          </Link>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="flex gap-2 flex-1 max-w-md">
            ...
            <button
              ...
              className="rounded-lg bg-primary-600 text-white px-3 py-2 hover:bg-primary-700"
            >
          ...
          <select
            value={classicFilter}
            onChange={(e) => setClassicFilter(e.target.value as "" | "true" | "false")}
            className="rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm"
          >
            <option value="">Все типы</option>
            <option value="true">Классика</option>
            <option value="false">Авторские</option>
          </select>
          <select
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-zinc-50 text-left text-zinc-600">
                <tr>
                  ...
                  <th className="px-4 py-3 hidden sm:table-cell">Тип</th>
                ...
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-zinc-50">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 shrink-0 rounded bg-zinc-100 flex items-center justify-center overflow-hidden">
                          {item.image_url ? (
                            ...
                            <Martini className="h-4 w-4 text-zinc-300" />
                          )}
                        </div>
                    ...
                    <td className="px-4 py-3 hidden sm:table-cell">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${item.is_classic ? "bg-primary-100 text-primary-700" : "bg-zinc-100 text-zinc-600"}`}>
            ...
            <div className="px-4 py-3 border-t border-zinc-100 text-xs text-zinc-500">
              Всего: {items.length}
            </div>
          </div>
        )}
