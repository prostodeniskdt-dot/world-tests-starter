...
export default function AdminCocktailEditPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/admin/cocktails/${cocktailId}`).then((r) => r.json()),
      fetch("/api/admin/cocktails/categories").then((r) => r.json()),
    ])
  ...
  return (
    <div className="min-h-screen bg-zinc-50">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <AdminCocktailEditForm initial={item} categories={categories} mode="edit" />
      </div>
    </div>
  );
}
