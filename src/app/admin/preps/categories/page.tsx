...
export default function AdminPrepCategoriesPage() {
  ...
  const handleCreate = async () => {
    ...
    setError(null);
    const res = await fetch("/api/admin/preps/categories", {
      method: "POST",
  ...
  const handleUpdate = async (id: number) => {
    ...
    setError(null);
    const res = await fetch(`/api/admin/preps/categories/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id: number) => {
    ...
    setError(null);
    const res = await fetch(`/api/admin/preps/categories/${id}`, { method: "DELETE" });
    const json = await res.json();
    if (json.ok) load();
    ...
    <div className="min-h-screen bg-zinc-50">
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-zinc-900">Категории заготовок</h1>
          <button
          ...
          <div className="bg-white rounded-xl border border-zinc-200 p-4 mb-4 flex gap-3 items-end">
            <div className="flex-1">
              ...
              <input
                ...
                className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500"
              />
            </div>
            <div className="w-24">
            ...
            <button onClick={handleCreate} className="text-green-600 hover:text-green-700 p-2">
              <Check className="h-5 w-5" />
            </button>
            <button onClick={() => setAdding(false)} className="text-zinc-400 hover:text-zinc-600 p-2">
              <X className="h-5 w-5" />
            </button>
          </div>
        )}
