...
export function GroupingQuestion({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Категории как визуальные корзины */}
      ...
      {/* Мобильная версия: кнопки для категорий */}
      <div className="block md:hidden space-y-4 mt-6">
        <div className="text-sm font-medium text-zinc-700 mb-2">
          Или выберите элемент и нажмите на категорию:
        </div>
        {question.items.map((item, itemIndex) => {
          const itemCats = getItemCategories(itemIndex);
          return (
            <div key={itemIndex} className="border-2 border-zinc-200 rounded-lg p-3 bg-white">
              <div className="font-medium mb-2 text-zinc-900 flex items-center gap-2">
                <span className="bg-zinc-200 text-zinc-700 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold">
                  {itemIndex + 1}
                </span>
                {item}
              </div>
                ...
                {question.categories.map((cat, catIdx) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={disabled}
                      className={`min-h-[40px] px-3 py-2 rounded-lg border-2 text-sm font-medium transition-all ${
                        isInCategory
