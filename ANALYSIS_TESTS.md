...
### Обратный перенос: БД → репозиторий

Чтобы выгрузить актуальные тесты из БД в папку `src/tests/` (например, для бэкапа или правок в коде):

```bash
npm run export-tests
```

Скрипт `scripts/export-tests-from-db.ts` читает все тесты из таблицы `tests`, записывает в `src/tests/{id}/public.ts` и `answer.ts`, и **удаляет из репо папки тестов, которых уже нет в БД**. Структура вопросов и `answerKey` не меняется — механики сохраняются.
