...
Подключение к БД берётся из `.env.local` (`DATABASE_URL`). Подходит для **PostgreSQL 18** (Timeweb Cloud и др.). Все скрипты запускаются из **корня проекта**.

Подробные консольные команды для миграций (в т.ч. первый запуск на Timeweb) — в файле **MIGRATION_COMMANDS.md**.

---

## ⚠️ Защита таблицы тестов

**`npm run run-db-migrations`** — безопасен для БД с данными. Не удаляет `tests`, выполняет только инкрементальные миграции.

**`npm run run-initial-tests-schema`** — **ОПАСНО**. Удаляет таблицу `tests` и все тесты. Только для:
- первичной настройки пустой БД;
- сознательного пересоздания схемы (с потерей данных).

---

## Безопасные инкрементальные миграции

Для БД, в которой уже есть тесты и данные. Не удаляет `tests`, добавляет только новые таблицы/колонки:
...
```bash
psql "$DATABASE_URL" -f db/migrations/20260216_tests_author.sql
```
